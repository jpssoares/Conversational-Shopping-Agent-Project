...
from data_utils.dataloading_functions import (
    qa_specific_dataloading,
    slot_filling_dataloading,
)
...
def get_model(checkpoint_name: str, tokenizer, accel) -> Tuple[Any, Any, Any]:
    ...
    if model_name == "bertdsti":
        model, dataloading_function = (
            BERTDSTI.BERTDSTI(checkpoint_name, tokenizer),
            slot_filling_dataloading,
        )
        input_function = get_simple_encoder_input_funcion(
            system_token="[sys]", slot_filling_to_prepend="[intent]"
        )
    else:
...
def get_simple_encoder_input_funcion(
    to_prepend="Yes . No . Dontcare .",
    system_token="[sys]",
    slot_filling_to_prepend="",
    **kwargs,
):
...
def get_simple_encoder_input_funcion_with_more_prepend(
    to_prepend="Yes . No . Dontcare .",
    system_token="[sys]",
    slot_filling_to_prepend="",
    **kwargs,
    ...
    return (
        model_input_functions.TypicalTransformerEncoderQAInputFunctionWithExtraSlotInfo(
            to_prepend=to_prepend,
            max_sequence_len=config.max_len,
            user_token="[usr]",
            system_token=system_token,
...
def get_qanlu_t5_prompted_input_function(to_prepend_a="", to_prepend_b=""):
    return model_input_functions.T5PPromptedInputFunction(
        ...
        user_token="; ",
        system_token=" ",
        to_prepend=to_prepend_a
        + " "
        + to_prepend_b,  # TODO this should be removed to facilitate more variety in prompting inputs
    )
